...
const Layout: React.FC = () => {
    ...
    const NavLink: React.FC<{ view: View, label: string, children?: React.ReactNode }> = ({ view, label }) => (
         <button 
            ...
            className={`w-full text-left px-4 py-2.5 rounded-lg transition-colors duration-200 text-sm font-medium ${currentView === view ? 'bg-indigo-600 text-white' : 'text-gray-300 hover:bg-gray-700 hover:text-white'}`}
        >
            {label}
        </button>
    );
    ...
    return (
        <div className="flex h-screen">
            <aside className="w-64 bg-gray-800 dark:bg-gray-900 p-4 space-y-4 flex flex-col shadow-lg">
            ...
            <main className="flex-1 p-10 overflow-y-auto bg-gray-50 dark:bg-gray-900/50">
                {renderView()}
            </main>
        </div>
    );
