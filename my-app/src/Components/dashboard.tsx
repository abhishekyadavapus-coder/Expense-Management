                        ...
                        {expense.approvalHistory.length > 0 ? expense.approvalHistory.map((entry, index) => (
                            <li key={index} className="flex items-start space-x-3">
                                <span className={`mt-1 flex h-6 w-6 items-center justify-center rounded-full ${entry.status === 'Approved' ? 'bg-green-500' : 'bg-red-500'}`}>
                                    {entry.status === 'Approved' ? ICONS.check : ICONS.x}
                                </span>
...
const ExpenseRow: React.FC<{ expense: Expense; isManagerView?: boolean }> = ({ expense, isManagerView = false }) => {
    ...
    const canTakeAction = useMemo(() => {
        ...
        // Find the role of the next required approver based on the workflow.
        const lastApproval = expense.approvalHistory.slice().reverse().find(h => h.status === 'Approved');
        let nextApproverRole: Role | null = null;

        if (!lastApproval) {
            // No approvals yet; requires the first person in the workflow.
            ...
            if (lastApproverIndex > -1 && lastApproverIndex < APPROVAL_WORKFLOW.length - 1) {
                // Determine the next approver in the sequence.
                nextApproverRole = APPROVAL_WORKFLOW[lastApproverIndex + 1];
            } else {
                // Already approved by the final person or last approver not in workflow.
                return false; 
            }
        }
        
        // Check if the current user has the required role for the next action.
        if (currentUser.role !== nextApproverRole) {
            return false;
        }

        // If the current user is a Manager, they can only approve expenses for their direct reports.
        if (currentUser.role === Role.Manager) {
            return employee?.managerId === currentUser.id;
        }

        // If the user is an Admin and it's their turn, they can approve.
        return true;

    }, [currentUser, expense, users, isManagerView]);
    
    const getConvertedAmount = useCallback(async () => {
        // 1. Handle same currency: No API call needed
        if (expense.currency === company.baseCurrency) {
            setConvertedAmount(expense.amount);
            setConversionError(false); // Ensure no error is shown
            return;
        }

        // 2. Handle different currency: Fetch rate
        setConversionError(false); // Reset error state on new fetch
        const rate = await getExchangeRate(expense.currency, company.baseCurrency);
        
        if (rate) {
            setConvertedAmount(expense.amount * rate);
        } else {
            // 3. Handle fetch failure: Set error state for clear UI feedback
            setConversionError(true);
            setConvertedAmount(null); // Clear any previous amount
        }
    ...
    return (
        <>
            <tr className="border-b dark:border-gray-700 bg-white dark:bg-gray-800 odd:bg-gray-50 dark:odd:bg-gray-800/50 hover:bg-gray-100 dark:hover:bg-gray-700/50 transition-colors">
...
const EmployeeDashboard = () => {
    ...
    // FIX: Widened the type of the filters state to be compatible with the FilterControls component.
    const [filters, setFilters] = useState<{ status?: string, category?: string, employee?: string }>({ status: 'All', category: 'All' });
    const myExpenses = useMemo(() => expenses.filter(e => e.userId === currentUser.id), [expenses, currentUser.id]);
    ...
    return (
        <div className="space-y-8">
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                <Card title="Pending Expenses"><p className="text-3xl font-bold">{pendingCount}</p></Card>
                <Card title="Approved Expenses"><p className="text-3xl font-bold text-emerald-500">{myExpenses.filter(e => e.status === 'Approved').length}</p></Card>
            </div>
...
const ManagerDashboard = () => {
    ...
    return (
         <div className="space-y-8">
            <Card title="Pending Team Approvals">
                <p className="text-3xl font-bold text-amber-500">{teamExpenses.filter(e => e.status === 'Pending').length}</p>
                <p className="text-sm text-gray-500 mt-1">Expenses from your team that need your review.</p>
            </Card>
    ...
    return (
        <div className="space-y-8">
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
                <Card title="Total Expenses"><p className="text-3xl font-bold">{stats.totalExpenses}</p></Card>
                <Card title="Pending"><p className="text-3xl font-bold text-amber-500">{stats.pending}</p></Card>
                <Card title="Approved"><p className="text-3xl font-bold text-emerald-500">{stats.approved}</p></Card>
                <Card title="Rejected"><p className="text-3xl font-bold text-red-500">{stats.rejected}</p></Card>
            </div>
